...
def neighbors(geohash_value):
    # 解码Geohash，获取对应的经纬度
    lat, lon = decode(geohash_value)
    
    # 定义相邻区域的经纬度偏移量，适当选择偏移值（比如0.001度）
    lat_offsets = [-0.001, 0, 0.001]
    ...
    for lat_offset in lat_offsets:
        for lon_offset in lon_offsets:
            if lat_offset == 0 and lon_offset == 0:
                continue  # 当前区域不算邻居
            # 计算新的经纬度
            new_lat = float(lat) + lat_offset
            new_lon = float(lon) + lon_offset
            # 编码新的经纬度为Geohash
            new_geohash = encode(new_lat, new_lon, precision=6)  # 选择合适的精度
            neighbors.append(new_geohash)
